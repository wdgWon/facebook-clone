...
// 버튼 활성,비활성 상태 path 패턴
const [HOME, FRIEND, WATCH, GROUP] = [
   {
      active:
...
// 네비게이션 버튼 스타일
const NavButtonStyle = ({ isActive, children }) => {
   const BUTTON_STYLE = "w-full -my-2 py-2 flex justify-center items-center";
   return (
      <button
         type="button"
         className={
            isActive
               ? `${BUTTON_STYLE} border-b-4 border-[#1b74e4]`
               : `${BUTTON_STYLE} rounded-md hover:bg-gray-300`
         }
      >
...
const HomeButton = ({ isActive }) => {
   return (
      <svg
         viewBox="0 0 28 28"
         className={isActive ? "text-[#1b74e4]" : "text-[#65676b]"}
         fill="currentColor"
         height="28"
         width="28"
      >
         <path d={isActive ? HOME.active : HOME.inactive}></path>
      </svg>
   );
};

// 친구
const FriendButton = ({ isActive }) => {
   return (
      <svg
         viewBox="0 0 28 28"
         className={isActive ? "text-[#1b74e4]" : "text-[#65676b]"}
         ...
         width="28"
      >
         <path d={isActive ? FRIEND.active : FRIEND.inactive}></path>
      </svg>
   );
};

// 동영상
const WatchButton = ({ isActive }) => {
   return (
      <svg
         viewBox="0 0 28 28"
         className={isActive ? "text-[#1b74e4]" : "text-[#65676b]"}
         ...
         width="28"
      >
         <path d={isActive ? WATCH.active : WATCH.inactive}></path>
      </svg>
   );
};

// 그룹
const GroupButton = ({ isActive }) => {
   return (
      <svg
         viewBox="0 0 28 28"
         className={isActive ? "text-[#1b74e4]" : "text-[#65676b]"}
         ...
         width="28"
      >
         <path d={isActive ? GROUP.active : GROUP.inactive}></path>
      </svg>
   );
};
...
export default function NavgationBar() {
   ...
   return (
      <>
         <nav id="navigation" className="sticky top-0 shadow-md py-2 w-screen z-10 bg-white">
            <div className="relative flex items-center justify-center w-full py-1 px-28">
               ...
               {/* 네비게이션 라우트 버튼 */}
               <div className="grid grid-flow-col justify-stretch space-x-4 w-[600px]">
                  <NavLink to="/" end>
                     {({ isActive }) => (
               ...
               <div className="absolute right-[30px]">
                  <div className="flex flex-row-reverse h-14 leading-4 items-center">
                     <NavProfile />
                     <Config />
                  </div>
               </div>
